...
所以在微前端的场景下，已经确定了其实多个站点的资源集合，无论是基座模式、去中心模式还是自组织模式，都离不开对站点资源的描述。

而 import-html-entry 就是为了实现对站点资源描述对象的提取。为了适配 qiankun，它还提供了对入口管理的功能。
...
- 从 html 里面解析获取到相关的 html、css 和 js
- 执行里面的脚本，获取到入口里的导出对象
